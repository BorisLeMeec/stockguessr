...
### Menu layout & duplicated settings

Menu order: global settings → daily card → TRAIN separator (`.zone-sep`) → training cards → rounds. Global settings (market+lang) are a single `.global-row` used on all sizes: top-right above the daily on desktop, one centered line with the `// MARKET` / `// LANG` labels stacked above their pills on mobile (≤700px, plus an extra squeeze at ≤359px so it fits a 320px screen without horizontal scroll). Rounds sits centered below the training grid on desktop (`.rounds-row`) and just above it on mobile (`.rounds-row-mobile`) — the rounds pill group is the only one that exists **twice** (`#rounds-pills` / `#rounds-pills-mobile`); the handler in `boot()` keeps both in sync. CSS gotcha: the ≤700px media block sits *earlier* in style.css than some base rules — use higher-specificity selectors (e.g. `#hint-pills`) for mobile overrides.
