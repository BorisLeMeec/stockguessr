...
Menu order: global settings → daily card → TRAIN separator (`.zone-sep`) → training cards → rounds. Global settings (market+lang) sit top-right above the daily on desktop (`.global-row`) and in the gear panel on mobile (≤700px); rounds sits centered below the training grid on desktop (`.rounds-row`) and just above it on mobile (`.rounds-row-mobile`). Each pill group exists **twice** (`#market-pills` / `#market-pills-mobile` etc.); handlers in `boot()` keep both in sync — any new setting must be added to both and wired the same way. The gear panel close-on-outside-tap uses a **capture-phase** listener that swallows the click so it can't also hit a card underneath. CSS gotcha: the ≤700px media block sits *earlier* in style.css than some base rules — use higher-specificity selectors (e.g. `#hint-pills`) for mobile overrides.
